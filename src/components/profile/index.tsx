...
import { SessionTitle } from "../sessonTitle"
import { toast } from "../toast"
import { WatchedTvShowComponent } from "../watchedTvshow"

interface ProfileProps {
  user: User
  watchedTvShows: WatchedTvShow[]
  ratings: Rating[]
  lists: ListWithTvShows[]
  favoriteTvShow?: TvShow
  favoriteActors: Actor[]
  createRating: (tvShowId: string, scaleId: number) => Promise<void>
...
export default function Profile({
  ...
  const regex = /^\/profile\/[0-9a-fA-F-]+$/

  const isProfilePage = regex.test(pathname)
            ...
            <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-5 gap-4">
              {favoriteActors?.length ? (
                ...
                <p className="text-gray-400 col-span-4">
                  Nenhum ator marcado como favorito.
                </p>
              )}
            </div>
          </div>
